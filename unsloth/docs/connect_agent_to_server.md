...
Провайдер в конфиге называется `server` (в пикере модели — `server/…`). Если на
той же машине крутится ещё и локальный инстанс, зарегистрируйте его отдельно с
`PROVIDER=host` (и своими `MODEL_REPO`/`MODEL_CTX`) — оба провайдера будут в
пикере одновременно, см. README, «Два инстанса». Скрипт работает и с
`opencode.json`, и с `opencode.jsonc` — пишет в уже существующий файл.

### Агент на том же сервере, где крутится Studio

Если `opencode` запускается прямо на сервере — команда та же, только
`<SERVER_IP>` = `127.0.0.1`, а ключ брать не нужно у коллеги: он уже лежит
на сервере в `out/agent_api_key` (его создал `setup.sh` при установке).
Из каталога репо на сервере:

```sh
bash setup.sh client 127.0.0.1 48218 "$(cat out/agent_api_key)" "$(cat out/model_display_name)"
```

(`out/model_display_name` тоже пишется при установке; если файла нет —
